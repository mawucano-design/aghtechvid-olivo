# app.py - Analizador de Viñedos y Olivares (basado en versión de palma, sin autenticación)
# 
...
# (Se incluyen todas las funciones de visualización del original, adaptadas)
...
with st.sidebar:
    ...
    crop_type = st.selectbox("Tipo de cultivo:", ["Vid", "Olivo"], index=0, key="crop_type_selector")
    st.session_state.crop_type = crop_type
    if crop_type == "Vid":
        variedad = st.selectbox("Variedad:", VARIEDADES_VID, index=0, key="variedad_selector")
    else:
        variedad = st.selectbox("Variedad:", VARIEDADES_OLIVO, index=0, key="variedad_selector")
    ...
    col1, col2 = st.columns(2)
    with col1:
        fecha_inicio = st.date_input("Inicio", datetime.now() - timedelta(days=60), key="fecha_inicio")
    with col2:
        fecha_fin = st.date_input("Fin", datetime.now(), key="fecha_fin")
    ...
    deteccion_habilitada = st.checkbox("Activar detección", value=True, key="deteccion_checkbox")
    if deteccion_habilitada:
        st.session_state.densidad_personalizada = st.slider("Densidad (plantas/ha):", 50, 200, 130, key="densidad_slider")
    ...
    uploaded_file = st.file_uploader("Archivo", type=['zip', 'kml', 'kmz', 'geojson'], key="file_uploader")
    if uploaded_file is not None:
        if st.button("🔄 Cargar", key="cargar_btn"):
            with st.spinner("Procesando..."):
                cargar_archivo_plantacion(uploaded_file)
                st.rerun()
    if st.session_state.get('archivo_cargado'):
        st.success(f"✅ Polígono cargado")
...
if st.session_state.archivo_cargado and st.session_state.gdf_original is not None:
    ...
    with col2:
        if st.button("🚀 EJECUTAR ANÁLISIS", use_container_width=True, key="ejecutar_analisis"):
            ...
            st.rerun()
        if deteccion_habilitada and st.button("🔍 DETECTAR PLANTAS", use_container_width=True, key="detectar_plantas"):
...
if st.session_state.analisis_completado:
    ...
    with tab1:
        ...
        salud_counts = gdf_completo['salud'].value_counts()
        ...
        with col_m3: st.metric("Edad Prom.", f"{edad_prom:.1f} años")
        with col_m4: st.metric("NDVI Prom.", f"{ndvi_prom:.3f}")
        with col_m5: st.metric("NDWI Prom.", f"{ndwi_prom:.3f}")
    ...
    with tab5:
        ...
        if st.session_state.deteccion_ejecutada and st.session_state.plantas_detectadas:
            ...
            total = len(plantas)
            area_total_val = st.session_state.resultados_todos['area_total']
