...
CULTIVOS = ['VID', 'OLIVO', 'TOMATE', 'CEBOLLA', 'PAPA', 'ZANAHORIA', 'LECHUGA', 'AJO']
...
def mostrar_analisis_economico(resultados_economicos):
    """Muestra resultados del análisis económico en formato tabular y métricas"""
    st.markdown("### 📈 RESULTADOS ECONÓMICOS")
    
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        st.metric("Rendimiento Actual", f"{resultados_economicos['rendimiento_actual_ton_ha']:.1f} ton/ha")
    with col2:
        st.metric("Rendimiento Proyectado", f"{resultados_economicos['rendimiento_proy_ton_ha']:.1f} ton/ha")
    with col3:
        st.metric("Incremento", f"+{resultados_economicos['incremento_rendimiento_ton_ha']:.1f} ton/ha")
    with col4:
        incremento_porcentaje = ((resultados_economicos['rendimiento_proy_ton_ha'] - resultados_economicos['rendimiento_actual_ton_ha']) / 
                                resultados_economicos['rendimiento_actual_ton_ha'] * 100) if resultados_economicos['rendimiento_actual_ton_ha'] > 0 else 0
        st.metric("Aumento %", f"+{incremento_porcentaje:.1f}%")
    
    st.markdown("#### 💰 FLUJO DE CAJA POR HECTÁREA")
    col_e1, col_e2, col_e3 = st.columns(3)
    with col_e1:
        st.markdown("**Ingresos**")
        st.write(f"- Actual: USD ${resultados_economicos['ingreso_actual_ha']:,.0f}")
        st.write(f"- Proyectado: USD ${resultados_economicos['ingreso_proy_ha']:,.0f}")
        st.write(f"- Incremento: USD ${resultados_economicos['incremento_margen_ha']:,.0f}")
    
    with col_e2:
        st.markdown("**Costos**")
        st.write(f"- Fertilización: USD ${resultados_economicos['costo_fertilizacion_ha']:,.0f}")
        st.write(f"- Insumos: USD ${resultados_economicos['costo_insumos_ha']:,.0f}")
        st.write(f"- Total: USD ${resultados_economicos['costo_total_ha']:,.0f}")
    
    with col_e3:
        st.markdown("**Rentabilidad**")
        st.write(f"- ROI Fertilización: {resultados_economicos['roi_fertilizacion_%']:.1f}%")
        st.write(f"- VAN Total: USD ${resultados_economicos['van_usd']:,.0f}")
        st.write(f"- TIR: {resultados_economicos['tir_%']:.1f}%")
    
    st.markdown("#### 📊 INDICADORES FINANCIEROS")
    df_economico = pd.DataFrame([
        {"Indicador": "Área Total (ha)", "Valor": f"{resultados_economicos['area_total_ha']:.1f}"},
        {"Indicador": "Incremento Producción Total (ton)", "Valor": f"{resultados_economicos['incremento_produccion_total_ton']:.1f}"},
        {"Indicador": "Incremento Ingreso Total (USD)", "Valor": f"${resultados_economicos['incremento_ingreso_total_usd']:,.0f}"},
        {"Indicador": "Costo Fertilización Total (USD)", "Valor": f"${resultados_economicos['costo_fertilizacion_total_usd']:,.0f}"},
        {"Indicador": "Relación B/C Actual", "Valor": f"{resultados_economicos['relacion_bc_actual']:.2f}"},
        {"Indicador": "Relación B/C Proyectada", "Valor": f"{resultados_economicos['relacion_bc_proy']:.2f}"},
        {"Indicador": "Punto de Equilibrio (ha)", "Valor": f"{resultados_economicos['punto_equilibrio_ha']:.1f}"}
    ])
    ...
    if resultados_economicos['roi_fertilizacion_%'] > 100:
        st.success(f"✅ **EXCELENTE RETORNO**: ROI del {resultados_economicos['roi_fertilizacion_%']:.1f}% indica alta rentabilidad de la fertilización")
    elif resultados_economicos['roi_fertilizacion_%'] > 50:
        st.info(f"⚠️ **BUEN RETORNO**: ROI del {resultados_economicos['roi_fertilizacion_%']:.1f}% justifica la inversión en fertilización")
    else:
        st.warning(f"⚠️ **RETORNO MODERADO**: ROI del {resultados_economicos['roi_fertilizacion_%']:.1f}% requiere análisis más detallado")
