...
def init_session_state():
    defaults = {
        ...
        'cultivo_seleccionado': 'Vid (Uva de Vino)',
        'textura_suelo': {},
        'textura_por_bloque': [],
        'datos_fertilidad': [],
        'analisis_suelo': True,
...
CULTIVOS_DISPONIBLES = [
    'Vid (Uva de Vino)', 'Vid (Uva de Mesa)', 'Olivo (Aceituna)', 
    'Olivo (Mesa)', 'Vid y Olivo Mixto'
]

VARIEDADES_VID = [
    'Cabernet Sauvignon', 'Merlot', 'Pinot Noir', 'Chardonnay', 
    'Sauvignon Blanc', 'Malbec', 'Tempranillo', 'Syrah', 
    'Garnacha', 'Riesling', 'Moscatel', 'Torrontés'
]

VARIEDADES_OLIVO = [
    'Arbequina', 'Picual', 'Hojiblanca', 'Cornicabra', 
    'Frantoio', 'Leccino', 'Koroneiki', 'Manzanilla', 
    'Gordal', 'Picudo', 'Empeltre', 'Arbosana'
]
...
def obtener_ndwi_earthdata(gdf_dividido, fecha_inicio, fecha_fin):
    """
    Obtiene NDWI real para cada bloque usando MOD09GA.
    """
    if not EARTHDATA_OK:
...
def mejorar_deteccion_plantas(gdf, densidad=400):
    """
    Densidad ajustada para vid (2000-4000 plantas/ha) y olivo (100-400 plantas/ha)
    """
    try:
        bounds = gdf.total_bounds
        ...
        for i in range(len(x_coords)):
            ...
            if plantacion_union.contains(point):
                ...
                plantas.append({
                    ...
                    'diametro_aprox': np.random.uniform(2, 4),
                    'simulado': True
                })
        return {
            'detectadas': plantas,
...
def ejecutar_deteccion_plantas():
    ...
    with st.spinner("Ejecutando detección MEJORADA de plantas..."):
        ...
        densidad = st.session_state.get('densidad_personalizada', 400)
    ...
    try:
        ...
        for idx, row in gdf.iterrows():
            ...
            if ndvi > 0.75:
                ...
                K = np.random.uniform(100, 150)
                pH = np.random.uniform(6.0, 7.0)
                MO = np.random.uniform(2.5, 4.0)
            elif ndvi > 0.5:
                N = np.random.uniform(50, 80)
                P = np.random.uniform(20, 30)
                K = np.random.uniform(70, 100)
                pH = np.random.uniform(5.5, 6.5)
                MO = np.random.uniform(2.0, 3.0)
            else:
                N = np.random.uniform(30, 50)
                P = np.random.uniform(10, 20)
                K = np.random.uniform(50, 70)
                pH = np.random.uniform(5.0, 5.8)
                MO = np.random.uniform(1.5, 2.5)
            
            if N < 60:
                rec_N = f"Aplicar {max(0, 60-N):.0f} kg/ha N (Urea: {max(0, (60-N)/0.46):.0f} kg/ha)"
            else:
                rec_N = "Mantener dosis actual"
            if P < 25:
                rec_P = f"Aplicar {max(0, 40-P):.0f} kg/ha P2O5 (DAP: {max(0, (40-P)/0.46):.0f} kg/ha)"
            else:
                rec_P = "Mantener dosis actual"
            if K < 80:
                rec_K = f"Aplicar {max(0, 120-K):.0f} kg/ha K2O (KCl: {max(0, (120-K)/0.6):.0f} kg/ha)"
            else:
...
def crear_mapa_fertilidad_interactivo(gdf_fertilidad, variable, colormap_nombre='YlOrRd'):
    info_var = {
        ...
        'P_kg_ha': {'titulo': 'Fósforo (P₂O₅)', 'unidad': 'kg/ha', 'vmin': 10, 'vmax': 50, 'cmap': 'YlOrRd'},
        'K_kg_ha': {'titulo': 'Potasio (K₂O)', 'unidad': 'kg/ha', 'vmin': 50, 'vmax': 150, 'cmap': 'YlGn'},
        'pH': {'titulo': 'pH del suelo', 'unidad': '', 'vmin': 5.0, 'vmax': 7.5, 'cmap': 'RdYlGn_r'},
...
def ejecutar_analisis_completo():
    ...
    # Clasificar salud (ajustado para vid y olivo)
    def clasificar_salud(ndvi):
        if ndvi < 0.3: return 'Crítica'
        if ndvi < 0.5: return 'Baja'
        if ndvi < 0.65: return 'Moderada'
        return 'Buena'
...
<div class="hero-banner">
    ...
    <p style="color: #a5d6a7; font-size: 1.1em;">
...
with st.sidebar:
    ...
    if deteccion_habilitada:
        if 'Vid' in cultivo:
            densidad_default = 2500
            densidad_min, densidad_max = 1000, 5000
        else:
            densidad_default = 200
            densidad_min, densidad_max = 50, 500
        densidad_personalizada = st.slider("Densidad objetivo (plantas/ha):", 
                                           densidad_min, densidad_max, densidad_default)
...
if st.session_state.archivo_cargado and st.session_state.gdf_original is not None:
    ...
    with col1:
        ...
        try:
            m_preview = folium.Map(location=[gdf.geometry.centroid.y.iloc[0], gdf.geometry.centroid.x.iloc[0]], 
                                  zoom_start=15, tiles=None)
            folium.TileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}',
                            attr='Esri', name='Satélite').add_to(m_preview)
            folium.GeoJson(gdf.to_json(), 
                          style_function=lambda x: {'fillColor': '#3388ff', 'color': 'black', 
                                                   'weight': 2, 'fillOpacity': 0.4}).add_to(m_preview)
    ...
    if gdf_completo is not None:
        ...
        with tab2:
            ...
            try:
                ...
                if st.session_state.get('plantas_detectadas'):
                    palmas_group = folium.FeatureGroup(name="Plantas detectadas")
                    for i, palma in enumerate(st.session_state.plantas_detectadas[:2000]):
        ...
        with tab5:
            ...
            if st.session_state.deteccion_ejecutada and st.session_state.get('plantas_detectadas'):
                ...
                try:
                    ...
                    folium.TileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', 
                                    attr='Esri', name='Satélite').add_to(m_palmas)
                    folium.GeoJson(gdf_completo.to_json(), 
                                  style_function=lambda x: {'color':'blue','fillOpacity':0.1}).add_to(m_palmas)
                    for i, palma in enumerate(plantas[:2000]):
                        if 'centroide' in palma:
                            lon, lat = palma['centroide']
                            folium.CircleMarker([lat, lon], radius=2, color='red', fill=True, 
                                                fill_color='red', fill_opacity=0.8,
                                                tooltip=f"Planta #{i+1}").add_to(m_palmas)
                ...
                if plantas:
                    try:
                        ...
                        gdf_palmas = gpd.GeoDataFrame(df_palmas, 
                        ...
                        with col_p1: st.download_button("🗺️ GeoJSON", geojson_palmas, f"plantas_{datetime.now():%Y%m%d}.geojson", "application/geo+json")
        ...
        with tab7:
            ...
            if textura_por_bloque:
                ...
                try:
                    ...
                    m_textura = folium.Map(location=[gdf_completo.geometry.centroid.y.mean(), 
                                                     gdf_completo.geometry.centroid.x.mean()], 
                                          zoom_start=15, tiles=None)
                    folium.TileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', 
...
Datos satelitales: NASA Earthdata · Clima: Open-Meteo ERA5 · Radiación/Viento: NASA POWER · Curvas de nivel: OpenTopography SRTM
Desarrollado por: BioMap Consultora | Contacto: mawucano@gmail.com | +5493525 532313
""", unsafe_allow_html=True)
