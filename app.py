# app.py - Versión final con correcciones de CRS para MODIS y Landsat
...
# - Umbral mínimo de píxeles por bloque configurable (por defecto 1)
#
...
st.set_page_config(
    page_title="Analizador de Olivo Satelital",
    page_icon="🌳",
    layout="wide",
...
# ===== CONFIGURACIONES =====
VARIEDADES_OLIVO = [
    'Arbequina', 'Picual', 'Frantoio', 'Manzanilla', 'Empeltre', 'Arauco',
    'Coratina', 'Leccino', 'Hojiblanca', 'Changlot Real', 'Criolla', 'Nevadillo',
...
# ===== FUNCIONES SATELITALES CORREGIDAS =====
def obtener_ndvi_earthdata(gdf_dividido, fecha_inicio, fecha_fin, min_pixels=1):
    if not EARTHDATA_OK:
    ...
    try:
        ...
        if RASTERIO_OK:
            try:
                    ...
                    if ndvi_sub:
                        with rasterio.open(ndvi_sub) as src_ndvi:
                            ...
                            if raster_crs is None:
                                raster_crs = rasterio.crs.CRS.from_proj4(
                            ...
                            for idx, row in gdf_proj.iterrows():
                                ...
                                try:
                                    ...
                                    if n_valid < min_pixels:
                                        st.info(f"Bloque {idx+1}: solo {n_valid} píxeles válidos (< {min_pixels}). Se asigna NaN.")
                                        ndvi_values.append(np.nan)
                                        continue
                                    mean_val = np.mean(data_scaled[mask_valid])
                ...
                st.warning(f"Fallo rasterio: {str(e)[:100]}. Intentando con xarray...")

        # --- Intentar con xarray + rioxarray ---
        if EARTHDATA_OK and 'xr' in globals():
            try:
                # Abrir con xarray usando rioxarray para acceder a subdatasets
                ds = xr.open_dataset(download_path, engine='netcdf4', group='MODIS_Grid_16DAY_250m_VI')
                # Buscar NDVI
                ndvi_var = None
                for var in ds.data_vars:
                    if 'NDVI' in var:
                        ...
                        break
                if ndvi_var is None:
                    st.error("No se encontró variable NDVI en el dataset.")
                    shutil.rmtree(temp_dir, ignore_errors=True)
                    return None
                ndvi = ds[ndvi_var]
                # Asignar CRS sinusoidal
                ndvi = ndvi.rio.write_crs("EPSG:3978")  # MODIS Sinusoidal
                # Recortar por polígonos
                gdf_proj = gdf_dividido.to_crs(ndvi.rio.crs)
                ndvi_clip = ndvi.rio.clip(gdf_proj.geometry.values, gdf_proj.crs)
                # Extraer valores por bloque
                ndvi_values = []
                for idx, row in gdf_proj.iterrows():
                    geom = row.geometry
                    # Recortar para cada geometría
                    ndvi_bloque = ndvi.rio.clip([geom], gdf_proj.crs, drop=False)
                    data = ndvi_bloque.values
                    if data.size == 0:
                        ndvi_values.append(np.nan)
                        continue
                    data_scaled = data * 0.0001
                    mask_valid = ~np.isnan(data_scaled) & (data_scaled >= -1) & (data_scaled <= 1)
                    n_valid = np.sum(mask_valid)
                    if n_valid < min_pixels:
                        st.info(f"Bloque {idx+1}: solo {n_valid} píxeles válidos (< {min_pixels}).")
                        ndvi_values.append(np.nan)
                        continue
                    mean_val = np.mean(data_scaled[mask_valid])
                    ndvi_values.append(round(float(mean_val), 3))
                ...
                st.warning(f"Fallo con xarray: {str(e)[:100]}. Intentando con pyhdf...")

        # --- Fallback con pyhdf usando lat/lon ---
        if PYHDF_OK:
            try:
                ...
                if ndvi_name is None:
                    st.error("No se encontró dataset NDVI.")
                    shutil.rmtree(temp_dir, ignore_errors=True)
                    return None
                if lat_name is None or lon_name is None:
                    # Intentar obtener lat/lon desde los atributos del archivo (transformación)
                    # Intentar construir transformación a partir de las dimensiones y la proyección sinusoidal
                    # Obtener dimensiones
                    ndvi_data = hdf.select(ndvi_name)
                    ydim, xdim = ndvi_data.info()[2]  # (dim0, dim1)
                    # Asumir que la extensión cubre todo el tile MODIS h13v12 (podríamos usar bounding box aproximado)
                    # Pero mejor usar el bounding box del polígono para inferir el área
                    bounds = gdf_dividido.total_bounds
                    minx, miny, maxx, maxy = bounds
                    # MODIS Sinusoidal: dimensiones globales? No es trivial.
                    # En su lugar, intentar leer los datasets de geolocalización con otro nombre
                    # Algunos productos tienen 'Lat' y 'Lon' o 'lat' 'lon'
                    for name in datasets.keys():
                        if 'Lat' in name:
                            lat_name = name
                        if 'Lon' in name:
                            lon_name = name
                    if lat_name is None or lon_name is None:
                        # No se encontraron, usar transformación aproximada
                        st.warning("No se encontraron datasets de latitud/longitud. Usando transformación aproximada a partir del bounding box.")
                        # Crear una transformación aproximada usando el bounding box y las dimensiones
                        # Nota: Esto asume que el tile cubre exactamente el área del polígono, lo cual no es cierto en general
                        # pero puede dar resultados aceptables si el polígono está dentro del tile.
                        lat_min, lat_max = miny, maxy
                        lon_min, lon_max = minx, maxx
                        res_lat = (lat_max - lat_min) / ydim
                        res_lon = (lon_max - lon_min) / xdim
                        transform = rasterio.Affine(res_lon, 0, lon_min, 0, res_lat, lat_min)
                        crs = rasterio.crs.CRS.from_epsg(4326)
                        # Usar esta transformación para crear un archivo en memoria
                        with rasterio.io.MemoryFile() as memfile:
                            with memfile.open(
                                ...
                                width=xdim,
                                count=1,
                                dtype=ndvi_data.info()[0],
                                crs=crs,
                                transform=transform,
                                nodata=-32768
                            ) as dst:
                                dst.write(ndvi_data.get(), 1)
                            with memfile.open() as src_ndvi:
                                ...
                                for idx, row in gdf_proj.iterrows():
                                    ...
                                    try:
                                        ...
                                        if out_image.ndim == 3 and out_image.shape[0] == 1:
                                            ...
                                            data = out_image
                                        data_scaled = data.astype(np.float32) * 0.0001
                                        mask_valid = (data != -32768) & (data_scaled >= -1) & (data_scaled <= 1)
                                        n_valid = np.sum(mask_valid)
                                        if n_valid < min_pixels:
                                            st.info(f"Bloque {idx+1}: solo {n_valid} píxeles válidos (< {min_pixels}). Se asigna NaN.")
                                            ndvi_values.append(np.nan)
                                ...
                                progress_bar.empty()
                                gdf_dividido['ndvi_modis'] = ndvi_values
                                st.success("✅ NDVI calculado por bloque correctamente con pyhdf y transformación aproximada.")
                                shutil.rmtree(temp_dir, ignore_errors=True)
                                return gdf_dividido
                    else:
                        # Tenemos lat/lon, proceder como antes
                        ndvi_data = hdf.select(ndvi_name).get().astype(np.float32) * 0.0001
                        lat_data = hdf.select(lat_name).get()
                        lon_data = hdf.select(lon_name).get()
                        ydim, xdim = ndvi_data.shape
                        # Construir transformación a partir de lat/lon
                        lat_00 = lat_data[0, 0]
                        lat_n0 = lat_data[-1, 0]
                        lon_00 = lon_data[0, 0]
                        lon_0n = lon_data[0, -1]
                        res_x = (lon_0n - lon_00) / xdim
                        res_y = (lat_n0 - lat_00) / ydim
                        transform = rasterio.Affine(res_x, 0, lon_00, 0, res_y, lat_00)
                        crs = rasterio.crs.CRS.from_epsg(4326)
                        with rasterio.io.MemoryFile() as memfile:
                            with memfile.open(
                                driver='GTiff',
                            ...
                            with memfile.open() as src_ndvi:
                                ...
                                for idx, row in gdf_proj.iterrows():
                                    ...
                                    try:
                                        ...
                                        if n_valid < min_pixels:
                                            ...
                                            ndvi_values.append(np.nan)
                                            continue
                                        mean_val = np.mean(data[mask_valid])
                                        ndvi_values.append(round(float(mean_val), 3))
                                    except Exception as e:
                                        ndvi_values.append(np.nan)
                                        st.warning(f"Error bloque {idx+1}: {str(e)[:100]}")
                                ...
                                progress_bar.empty()
                                gdf_dividido['ndvi_modis'] = ndvi_values
                                st.success("✅ NDVI calculado por bloque correctamente con pyhdf y geolocalización.")
                                shutil.rmtree(temp_dir, ignore_errors=True)
                                return gdf_dividido
            ...
            st.error("No se pudo leer el archivo HDF: ni rasterio ni xarray ni pyhdf están disponibles o funcionaron.")
            shutil.rmtree(temp_dir, ignore_errors=True)
            return None
    except Exception as e:
...
def obtener_ndwi_earthdata(gdf_dividido, fecha_inicio, fecha_fin, min_pixels=1):
    if not EARTHDATA_OK:
        st.error("Librerías earthaccess/xarray/rioxarray no instaladas.")
        return None
    if not EARTHDATA_USERNAME or not EARTHDATA_PASSWORD:
    ...
    try:
                            ...
                            for idx, row in gdf_proj.iterrows():
                                ...
                                try:
                                    ...
                                    if n_valid < min_pixels:
                                        ...
                                        ndwi_values.append(np.nan)
                                        continue
                                    ndwi = (nir_band[valid] - swir_band[valid]) / (nir_band[valid] + swir_band[valid])
                                    mean_val = np.mean(ndwi)
        ...
        if EARTHDATA_OK and 'xr' in globals():
            try:
                ds = xr.open_dataset(download_path, engine='netcdf4', group='MODIS_Grid_1km_2d')
                nir_var = None
                swir_var = None
                for var in ds.data_vars:
                ...
                if nir_var is None or swir_var is None:
                    st.error("No se encontraron variables NIR/SWIR.")
                    shutil.rmtree(temp_dir, ignore_errors=True)
                    return None
                nir = ds[nir_var]
                swir = ds[swir_var]
                nir = nir.rio.write_crs("EPSG:3978")
                swir = swir.rio.write_crs("EPSG:3978")
                gdf_proj = gdf_dividido.to_crs(nir.rio.crs)
                ndwi_values = []
                for idx, row in gdf_proj.iterrows():
                    geom = row.geometry
                    nir_clip = nir.rio.clip([geom], gdf_proj.crs, drop=False)
                    swir_clip = swir.rio.clip([geom], gdf_proj.crs, drop=False)
                    if nir_clip.size == 0 or swir_clip.size == 0:
                        ndwi_values.append(np.nan)
                        continue
                    nir_data = nir_clip.values * 0.0001
                    swir_data = swir_clip.values * 0.0001
                    valid = ~np.isnan(nir_data) & ~np.isnan(swir_data) & (nir_data + swir_data != 0)
                    n_valid = np.sum(valid)
                    if n_valid < min_pixels:
                        st.info(f"Bloque {idx+1}: solo {n_valid} píxeles válidos (< {min_pixels}).")
                        ndwi_values.append(np.nan)
                        continue
                    ndwi = (nir_data[valid] - swir_data[valid]) / (nir_data[valid] + swir_data[valid])
                    mean_val = np.mean(ndwi)
                    ndwi_values.append(round(float(mean_val), 3))
                gdf_dividido['ndwi_modis'] = ndwi_values
            ...
            try:
                ...
                for name in datasets.keys():
                    if 'sur_refl_b02' in name:
                        ...
                        lon_name = name
                if nir_name is None or swir_name is None:
                    st.error("No se encontraron datasets NIR o SWIR.")
                    shutil.rmtree(temp_dir, ignore_errors=True)
                    return None
                if lat_name is None or lon_name is None:
                    # Intentar construir transformación aproximada
                    nir_data = hdf.select(nir_name)
                    ydim, xdim = nir_data.info()[2]
                    bounds = gdf_dividido.total_bounds
                    minx, miny, maxx, maxy = bounds
                    res_lat = (maxy - miny) / ydim
                    res_lon = (maxx - minx) / xdim
                    transform = rasterio.Affine(res_lon, 0, minx, 0, res_lat, miny)
                    crs = rasterio.crs.CRS.from_epsg(4326)
                    with rasterio.io.MemoryFile() as memfile_nir, rasterio.io.MemoryFile() as memfile_swir:
                        with memfile_nir.open(driver='GTiff', height=ydim, width=xdim, count=1,
                                              dtype=nir_data.info()[0], crs=crs, transform=transform, nodata=-32768) as dst_nir:
                            dst_nir.write(nir_data.get(), 1)
                        swir_data = hdf.select(swir_name)
                        with memfile_swir.open(driver='GTiff', height=ydim, width=xdim, count=1,
                                               dtype=swir_data.info()[0], crs=crs, transform=transform, nodata=-32768) as dst_swir:
                            dst_swir.write(swir_data.get(), 1)
                        with memfile_nir.open() as src_nir, memfile_swir.open() as src_swir:
                            ...
                            for idx, row in gdf_proj.iterrows():
                                ...
                                try:
                                    ...
                                    if out_swir.ndim == 3 and out_swir.shape[0] == 1:
                                        ...
                                        swir_band = out_swir.astype(np.float32) * 0.0001
                                    valid = (nir_band != -32768 * 0.0001) & (swir_band != -32768 * 0.0001) & (nir_band + swir_band != 0)
                                    n_valid = np.sum(valid)
                                    if n_valid < min_pixels:
                                        st.info(f"Bloque {idx+1}: solo {n_valid} píxeles válidos (< {min_pixels}). Se asigna NaN.")
                                        ndwi_values.append(np.nan)
                            ...
                            progress_bar.empty()
                            gdf_dividido['ndwi_modis'] = ndwi_values
                            st.success("✅ NDWI calculado por bloque correctamente con pyhdf y transformación aproximada.")
                            shutil.rmtree(temp_dir, ignore_errors=True)
                            return gdf_dividido
                else:
                    # Usar lat/lon reales
                    nir_data = hdf.select(nir_name).get().astype(np.float32) * 0.0001
                    swir_data = hdf.select(swir_name).get().astype(np.float32) * 0.0001
                    lat_data = hdf.select(lat_name).get()
                    lon_data = hdf.select(lon_name).get()
                    ...
                    with rasterio.io.MemoryFile() as memfile_nir, rasterio.io.MemoryFile() as memfile_swir:
                        ...
                        with memfile_nir.open() as src_nir, memfile_swir.open() as src_swir:
                            ...
                            progress_bar.empty()
                            gdf_dividido['ndwi_modis'] = ndwi_values
                            st.success("✅ NDWI calculado por bloque correctamente con pyhdf y geolocalización.")
                            shutil.rmtree(temp_dir, ignore_errors=True)
                            return gdf_dividido
            except Exception as e_pyhdf:
                st.error(f"Error al procesar con pyhdf: {str(e_pyhdf)}")
...
def obtener_ndvi_ndwi_landsat(gdf_dividido, fecha_inicio, fecha_fin, max_cloud=100, expand_bbox=0.10, min_pixels=1):
    if not EARTHDATA_OK:
        st.error("Librerías earthaccess no instaladas.")
        return None
    if not RASTERIO_OK:
                ...
                for i, row in gdf_proj.iterrows():
                    ...
                    try:
                        ...
                        for name, arr in [("B4", out_b4), ("B5", out_b5), ("B6", out_b6)]:
                            if arr.ndim < 2:
                                raise ValueError(f"Array {name} es {arr.ndim}D")
                        ...
                        valid = valid_mask & (red >= 0) & (red <= 1) & (nir >= 0) & (nir <= 1) & (swir >= 0) & (swir <= 1)
...
def mejorar_deteccion_olivos(gdf, densidad=200):
    try:
        bounds = gdf.total_bounds
        ...
        num_olivos_objetivo = int(area_ha * densidad)
        ...
        olivos = []
        for i in range(len(x_coords)):
            if len(olivos) >= num_olivos_objetivo:
                break
            ...
            if plantacion_union.contains(point):
                ...
                olivos.append({
                    'centroide': (lon, lat),
                    'area_m2': np.random.uniform(8, 15),
                    'circularidad': np.random.uniform(0.8, 0.95),
                    'diametro_aprox': np.random.uniform(3, 5),
                    'simulado': True
                })
        return {
            'detectadas': olivos,
            ...
            'densidad_calculada': len(olivos) / area_ha,
            'area_ha': area_ha
        }
    except Exception as e:
...
def ejecutar_deteccion_olivos():
    if st.session_state.gdf_original is None:
        st.error("Primero debe cargar un archivo de plantación")
        return
    with st.spinner("Ejecutando detección MEJORADA de olivos..."):
        gdf = st.session_state.gdf_original
        densidad = st.session_state.get('densidad_personalizada', 200)
        resultados = mejorar_deteccion_olivos(gdf, densidad)
        olivos_verificados = verificar_puntos_en_poligono(resultados['detectadas'], gdf)
        st.session_state.olivos_detectados = olivos_verificados
        st.session_state.deteccion_ejecutada = True
        st.success(f"✅ Detección MEJORADA completada: {len(olivos_verificados)} olivos detectados")
    ...
    try:
        ...
        caracteristicas = {
            'Franco Arcilloso': {
                ...
                'recomendacion': 'Ideal para olivo'
            },
    ...
    try:
        ...
        for idx, row in gdf.iterrows():
            ...
            if ndvi > 0.75:
                ...
                MO = np.random.uniform(2.5, 4.0)
            elif ndvi > 0.6:
                N = np.random.uniform(50, 80)
                P = np.random.uniform(20, 30)
                K = np.random.uniform(80, 120)
                pH = np.random.uniform(5.5, 6.0)
                MO = np.random.uniform(1.8, 2.5)
            else:
                N = np.random.uniform(30, 50)
                P = np.random.uniform(10, 20)
                K = np.random.uniform(50, 80)
                pH = np.random.uniform(5.0, 5.5)
                MO = np.random.uniform(1.0, 1.8)
            
            if N < 70:
                rec_N = f"Aplicar {max(0, 100-N):.0f} kg/ha N (Urea: {max(0, (100-N)/0.46):.0f} kg/ha)"
            else:
            ...
            if K < 100:
                rec_K = f"Aplicar {max(0, 150-K):.0f} kg/ha K2O (KCl: {max(0, (150-K)/0.6):.0f} kg/ha)"
            else:
...
def crear_mapa_fertilidad_interactivo(gdf_fertilidad, variable, colormap_nombre='YlOrRd'):
    info_var = {
        'N_kg_ha': {'titulo': 'Nitrógeno (N)', 'unidad': 'kg/ha', 'vmin': 30, 'vmax': 120, 'cmap': 'YlGnBu'},
        'P_kg_ha': {'titulo': 'Fósforo (P₂O₅)', 'unidad': 'kg/ha', 'vmin': 10, 'vmax': 60, 'cmap': 'YlOrRd'},
        'K_kg_ha': {'titulo': 'Potasio (K₂O)', 'unidad': 'kg/ha', 'vmin': 50, 'vmax': 180, 'cmap': 'YlGn'},
        'pH': {'titulo': 'pH del suelo', 'unidad': '', 'vmin': 4.5, 'vmax': 7.0, 'cmap': 'RdYlGn_r'},
        'MO_porcentaje': {'titulo': 'Materia Orgánica', 'unidad': '%', 'vmin': 1.0, 'vmax': 4.0, 'cmap': 'BrBG'}
    }
...
def ejecutar_analisis_completo():
    ...
    with st.spinner("Ejecutando análisis completo..."):
        ...
        min_pixels = st.session_state.get('min_pixels', 1)
        ...
        if source == 'Landsat (30m)':
            ...
            resultado_ndvi = obtener_ndvi_earthdata(gdf_dividido, fecha_inicio, fecha_fin, min_pixels=min_pixels)
            if resultado_ndvi is None:
                st.error("No se pudo obtener NDVI real. Verifique sus credenciales y conexión.")
                st.stop()
            gdf_dividido = resultado_ndvi
        ...
        # Verificar si hay algún dato válido
        if 'ndvi_modis' in gdf_dividido.columns and gdf_dividido['ndvi_modis'].notna().any():
            st.success(f"✅ Se calcularon NDVI para {gdf_dividido['ndvi_modis'].notna().sum()} bloques de {len(gdf_dividido)}.")
        else:
            st.error("❌ No se pudo calcular NDVI para ningún bloque. Revise el polígono y la fecha.")
            st.stop()
...
with st.sidebar:
    ...
    st.slider("Mínimo de píxeles válidos:", 1, 30, 1, key="min_pixels",
              help="Si un bloque tiene menos de este número de píxeles válidos, se asigna NaN. Con 1 se acepta cualquier bloque que contenga al menos un píxel.")
    ...
    st.markdown("---")
    st.markdown("### 🌳 Detección de Olivos")
    deteccion_habilitada = st.checkbox("Activar detección de árboles", value=True)
    if deteccion_habilitada:
        densidad_personalizada = st.slider("Densidad objetivo (olivos/ha):", 100, 300, 200)
...
if st.session_state.archivo_cargado and st.session_state.gdf_original is not None:
    ...
    with col2:
        ...
        with col_btn2:
            if deteccion_habilitada:
                if st.button("🔍 DETECTAR OLIVOS", use_container_width=True):
                    ejecutar_deteccion_olivos()
                    st.rerun()
else:
    ...
    if gdf_completo is not None:
        ...
        with tab2:
            ...
            try:
                ...
                if st.session_state.olivos_detectados:
                    olivos_group = folium.FeatureGroup(name="Olivos detectados")
                    for i, olivo in enumerate(st.session_state.olivos_detectados[:2000]):
                        if 'centroide' in olivo:
                            lon, lat = olivo['centroide']
                            folium.CircleMarker([lat, lon], radius=2, color='red', fill=True,
                                                fill_color='red', fill_opacity=0.8).add_to(olivos_group)
                    olivos_group.add_to(mapa_interactivo)
        ...
        with tab5:
            st.subheader("🌳 DETECCIÓN DE OLIVOS INDIVIDUALES")
            if st.session_state.deteccion_ejecutada and st.session_state.olivos_detectados:
                olivos = st.session_state.olivos_detectados
                total = len(olivos)
                area_total_val = resultados.get('area_total', 0)
                densidad = total / area_total_val if area_total_val > 0 else 0
                st.success(f"✅ Detección completada: {total} olivos detectados")
                col1, col2, col3, col4 = st.columns(4)
                with col1: st.metric("Olivos detectados", f"{total:,}")
                with col2: st.metric("Densidad", f"{densidad:.0f} plantas/ha")
                with col3: st.metric("Área promedio", f"{np.mean([o.get('area_m2',0) for o in olivos]):.1f} m²")
                with col4: st.metric("Diámetro promedio", f"{np.mean([o.get('diametro_aprox',0) for o in olivos]):.1f} m")
                st.markdown("### 🗺️ Mapa de Distribución")
                try:
                    centroide = gdf_completo.geometry.unary_union.centroid
                    m_olivos = folium.Map(location=[centroide.y, centroide.x], zoom_start=16, tiles=None)
                    folium.TileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}', attr='Esri', name='Satélite').add_to(m_olivos)
                    folium.GeoJson(gdf_completo.to_json(), style_function=lambda x: {'color':'blue','fillOpacity':0.1}).add_to(m_olivos)
                    for i, olivo in enumerate(olivos[:2000]):
                        if 'centroide' in olivo:
                            lon, lat = olivo['centroide']
                            folium.CircleMarker([lat, lon], radius=2, color='red', fill=True, 
                                                fill_color='red', fill_opacity=0.8,
                                                tooltip=f"Olivo #{i+1}").add_to(m_olivos)
                    folium.LayerControl().add_to(m_olivos); Fullscreen().add_to(m_olivos)
                    folium_static(m_olivos, width=1000, height=600)
                except Exception as e:
                    st.error(f"Error al mostrar mapa de olivos: {str(e)[:100]}")
                if olivos:
                    try:
                        df_olivos = pd.DataFrame([{
                            'id': i+1, 'longitud': o.get('centroide', (0,0))[0], 'latitud': o.get('centroide', (0,0))[1],
                            'area_m2': o.get('area_m2', 0), 'diametro_m': o.get('diametro_aprox', 0)
                        } for i,o in enumerate(olivos)])
                        gdf_olivos = gpd.GeoDataFrame(df_olivos, geometry=gpd.points_from_xy(df_olivos.longitud, df_olivos.latitud), crs='EPSG:4326')
                        geojson_olivos = gdf_olivos.to_json(); csv_olivos = df_olivos.to_csv(index=False)
                        col_p1, col_p2 = st.columns(2)
                        with col_p1: st.download_button("🗺️ GeoJSON", geojson_olivos, f"olivos_{datetime.now():%Y%m%d}.geojson", "application/geo+json")
                        with col_p2: st.download_button("📊 CSV", csv_olivos, f"coordenadas_{datetime.now():%Y%m%d}.csv", "text/csv")
                    except: st.info("No se pudieron exportar los datos")
            else:
                st.info("La detección de olivos no se ha ejecutado aún.")
                if st.button("🔍 EJECUTAR DETECCIÓN DE OLIVOS", key="detectar_olivos_tab5", use_container_width=True):
                    ejecutar_deteccion_olivos()
                    st.rerun()
